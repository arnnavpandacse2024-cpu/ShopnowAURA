...
function syncStorage() {
  ...
  fetch('/api/state', {
    ...
    body: JSON.stringify({
      shops, deliveries, offers: activeOffers, totalCouriersPool,
  ...
  trending.forEach(product => {
    const pCard = `
      <div class="dash-cat-card" style="padding: 12px; align-items: stretch; min-width: unset; text-align: left; cursor: default;">
        <div class="placeholder-img" style="height: 120px; border-radius: var(--border-radius-sm); margin-bottom: 12px;">
  ...
  filteredProducts.forEach(p => {
    ...
    const itemHTML = `
      <div class="product-card glass-card" style="padding: 20px; display: flex; flex-direction: column;">
        <div class="product-rating" style="margin-bottom: 12px; display: flex; align-items: center;">
          ${generateRatingStars(p.rating)}
          <span class="rating-count" style="margin-left: 6px;">(${p.reviews})</span>
          ${discountBadgeHTML}
        </div>
      ...
      cart.forEach((item, index) => {
        ...
        dashCartItems.innerHTML += `
          <div class="cart-item-sm">
            <div class="placeholder-img cart-item-sm-img">
              <i data-lucide="image" style="width:16px;height:16px;opacity:0.3;"></i>
            </div>
